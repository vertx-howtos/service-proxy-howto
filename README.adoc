...
First, we must define our service interface. Behind the hoods, a code generator analyze it and generates the event bus message handler and the proxy class.

This is the `BarmanService` interface:
[source,java,indent=0,role="collapsed"]
----
...
. How to design and implement an Event Bus Service using Vert.x Service Proxy
. How to expose the service on the event bus
